...
module.exports = class Collection {
	constructor(redi, { database, collection }, options) {
		...
		if (this.redi.method == 'WS') {
			...
			setInterval(() => {
				Array.from(this.queue.values()).filter(queue => queue.elaps <= Date.now()).forEach(request => {
					if (request.retries + 1 > this.options.maxRetries) {
						this.queue.delete(request.updateID)

						const error = new Error(`Request ${request.updateID} was not processed: server did not respond after ${request.retries}/${this.options.maxRetries} requests`);
						request.reject(error)

						return this.redi.emit('error', { inputData: null, error })
					}

					if (this.queue.has(request.updateID)) {
						this.queue.set(request.updateID, { ...request, retries: request.retries + 1, })
						request.afterElaps()
					}
				});
			}, this.options.retryAfter * 1000)
		}
	}
	...
	async create(...objects) {
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				retries: 0,
				elaps: Date.now() + this.options.retryAfter * 1000,
				afterElaps: async () => {
					if (!this.queue.has(requestID)) return;
					this.queue.delete(requestID)

					const data = await this.create(objects);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise(async (resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.search(filter, requestID);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.searchOne(filter);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.update(filter, update);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.instantUpdate(filter, update);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.searchOrCreate(filter, create);
					resolve(data)
				},
		...
		if (this.redi.method == 'WS') return new Promise((resolve, reject) => {
			...
			this.queue.set(requestID, {
				...
				afterElaps: async () => {
					...
					const data = await this.delete(filter);
					resolve(data)
				},
